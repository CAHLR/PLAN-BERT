...
class _BaseMultiHeadAttention(Layer):
    ...
    Self-attention and its more general form used in decoders (the one whichF
    ...
    def validate_model_dimensionality(self, d_model: int):
        if d_model % self.num_heads != 0:
            raise ValueError('The size of the last dimension of the input ({0}) must be evenly divisible by the number of the attention heads {1}'.format(d_model, self.num_heads))
    ...
    def attention(self, pre_q, pre_v, pre_k, out_seq_len: int, d_model: int,
        ...
        weights = self.mask_attention_if_needed(
                K.batch_dot(
                    K.reshape(q, (-1,) + q_shape[-2:]),
                    K.reshape(k_transposed,(-1,) + k_t_shape[-2:])
                )/ sqrt_d)
        attention_heads = K.reshape(
            K.batch_dot(
                self.apply_dropout_if_needed(K.softmax(weights), training=training),
...
class MultiHeadAttention(_BaseMultiHeadAttention):
    ...
    def build(self, input_shape):
        ...
        if query_dim != values_dim:
            raise ValueError(
                'Both keys/value and query inputs must be of the same dimensionality, instead of {0} and {1}.'.format(values_dim, query_dim))
        d_model = query_dim
    ...
    def call(self, inputs, **kwargs):
        ...
        return self.attention(pre_q, pre_v, pre_k, query_seq_len, d_model,
                              training=kwargs.get('training'))[0]
...
class MultiHeadSelfAttention(_BaseMultiHeadAttention):
    ...
    def call(self, inputs, **kwargs):
        ...
        pre_q, pre_k, pre_v = [
            ...
            for i in range(3)]
        attention_out, weight = self.attention(pre_q, pre_v, pre_k, seq_len, d_model,
                                       training=kwargs.get('training'))
        return [attention_out, K.mean(weight, axis=0, keepdims=True)]
    
    def compute_output_shape(self, input_shape):
        return [input_shape, [None,12,12]]
